...
/*[clinic end generated code: output=5fc29463c3936a9c input=e7c8f4ba6dbbadd3]*/
{
    int ncpu = 0;
#ifdef MS_WINDOWS
    /* If GetActiveProcessorCount is called directly, Python crashes on Windows Vista 
    because this function does not exist in kernel32.dll of that Windows version. 
    For this reason, the availability of the function is checked at runtime with GetProcAddress. 
    If it is not available (the case of Windows Vista), this is handled gracefully without 
    crashing the interpreter. */
    HMODULE hKernel32 = GetModuleHandle("kernel32.dll");
    typedef DWORD (WINAPI *LPFN_GETACTIVEPROCESSORCOUNT)(WORD);
    LPFN_GETACTIVEPROCESSORCOUNT lpfn_GetActiveProcessorCount = (LPFN_GETACTIVEPROCESSORCOUNT)
        GetProcAddress(hKernel32, "GetActiveProcessorCount");
    if (lpfn_GetActiveProcessorCount != NULL) {
        ncpu = lpfn_GetActiveProcessorCount(ALL_PROCESSOR_GROUPS);
    } else { // GetActiveProcessorCount function is not available
        /* Retrieve the number of processors in the alternative way. */
